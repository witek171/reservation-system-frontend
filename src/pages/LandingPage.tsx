...
// previewed.app, shots.so
  return (
    <div className="bg-background min-h-screen flex flex-col">
      {/* Header */}
      <header className="w-full h-16 border-b border-surface-variant bg-surface-container-lowest/80 backdrop-blur-md fixed top-0 z-50">
        <div className="max-w-7xl mx-auto px-gutter h-full flex items-center justify-between">
          <div className="font-h3 text-h3 text-primary tracking-wider font-black">
            Bookium
          </div>
        </div>
      </header>
        ...
        <section className="bg-surface-container-low border-t border-surface-variant py-xl">
          <div className="max-w-7xl mx-auto px-gutter">
            <div className="flex flex-col items-center text-center mb-lg">
              <h2 className="font-h2 text-h2 text-on-surface mb-xs">Kluczowe Moduły</h2>
              <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl">
                Skonfiguruj system pod własne potrzeby. Minimalistyczny interfejs redukuje zmęczenie poznawcze personelu.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
              {/* Feature 1: Grafik (Large) */}
              <div className="md:col-span-8 bg-surface-container-lowest rounded-xl p-lg border border-surface-variant shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between group overflow-hidden relative">
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary-fixed-dim/20 rounded-full blur-3xl -mr-16 -mt-16 transition-transform group-hover:scale-110"></div>
                <div className="relative z-10 flex flex-col h-full">
                  <div className="w-12 h-12 rounded-lg bg-primary-fixed flex items-center justify-center text-on-primary-fixed mb-md">
                    <span className="material-symbols-outlined text-[24px]">calendar_view_week</span>
                  </div>
                  <h3 className="font-h3 text-h3 text-on-surface mb-sm">{t('landing.features.schedule')}</h3>
              ...
              {/* Feature 2: Zespół (Medium) */}
              <div className="md:col-span-4 bg-surface-container-lowest rounded-xl p-lg border border-surface-variant shadow-sm hover:shadow-md transition-shadow flex flex-col">
                <div className="w-12 h-12 rounded-lg bg-surface-container-high flex items-center justify-center text-on-surface mb-md">
                  <span className="material-symbols-outlined text-[24px]">groups</span>
                </div>
                <h3 className="font-h3 text-h3 text-on-surface mb-sm">{t('landing.features.team')}</h3>
                <p className="font-body-md text-body-md text-on-surface-variant">
                  {t('landing.features.teamDesc')}
                </p>
              </div>

              {/* Feature 3: Płatności (Wide) */}
              <div className="md:col-span-12 bg-tertiary-container rounded-xl p-lg shadow-sm overflow-hidden relative flex items-center min-h-[200px]">
                <div className="absolute inset-0 bg-gradient-to-r from-tertiary-container to-tertiary-fixed-dim/40"></div>
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between w-full gap-md">
                  <div className="flex flex-col max-w-xl">
                    <div className="w-12 h-12 rounded-lg bg-on-tertiary-container/10 flex items-center justify-center text-on-tertiary mb-md">
                      <span className="material-symbols-outlined text-[24px]">payments</span>
                    </div>
                    <h3 className="font-h3 text-h3 text-on-tertiary mb-sm">{t('landing.features.payments')}</h3>
                    <p className="font-body-md text-body-md text-tertiary-fixed-dim">
                      {t('landing.features.paymentsDesc')}
                    </p>
                  </div>
                  <div className="hidden md:flex gap-sm">
                    <div className="w-16 h-16 rounded-full border border-tertiary-fixed/30 flex items-center justify-center text-tertiary-fixed opacity-50">
                      <span className="material-symbols-outlined">sync_alt</span>
                    </div>
                    <div className="w-16 h-16 rounded-full border border-tertiary-fixed/30 flex items-center justify-center text-tertiary-fixed">
